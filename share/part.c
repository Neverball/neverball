...
static void part_init_goal(double h)
{
    ...
    for (i = 0; i < PART_MAX_GOAL; i++)
    {
        ...
        part_goal[i].p[0] = sin(a);
        part_goal[i].p[1] = (1.0 - t) * h;
        part_goal[i].p[2] = cos(a);
...
static void part_spin(struct part *part, int n, const double *g, double dt)
{
    ...
    for (i = 0; i < n; i++)
        if (part[i].t > 0.0)
        {
            ...
            part[i].p[0] = sin(V_RAD(part[i].a));
            part[i].p[2] = cos(V_RAD(part[i].a));
        }
}
...
static void part_draw(const double p[3], const double c[3],
                      double a, double r, double rx, double ry, double rz)
{
    glPushMatrix();
    {
        glTranslated(r * p[0], p[1], r * p[2]);
        ...
        for (i = 0; i < PART_MAX_COIN; i++)
            if (part_coin[i].t > 0.0)
                part_draw(part_coin[i].p,
                          part_coin[i].c,
                          part_coin[i].t,
                          1.0, rx, ry, r * part_coin[i].w);
    }
    glPopAttrib();
    glPopAttrib();
}

void part_draw_goal(double rx, double ry, double radius)
{
    double r = (double) SDL_GetTicks() / 1000.0;
    int i;
        ...
        for (i = 0; i < PART_MAX_GOAL; i++)
            if (part_goal[i].t > 0.0)
                part_draw(part_goal[i].p,
                          part_goal[i].c,
                          part_goal[i].t,
                          radius - 0.05, rx, ry, r * part_goal[i].w);
    }
    glPopAttrib();
    glPopAttrib();
}
