...
int sol_load_vary(struct s_vary *fp, struct s_base *base)
{
    ...
    if (fp->base->pc)
    {
        fp->pv = (v_path *)calloc(fp->base->pc, sizeof (*fp->pv));
        fp->pc = fp->base->pc;
    ...
    if (fp->base->bc)
    {
        ...
        fp->bv = (v_body *)calloc(fp->base->bc, sizeof (*fp->bv));
        fp->bc = fp->base->bc;
        ...
        for (i = 0; i < fp->base->bc; i++)
        {
            ...
            if (bbody->pi >= 0 && (vmove = (v_move *)alloc_add(&mv)))
            {
            ...
            else if (bbody->pj >= 0 && (vmove = (v_move *)alloc_add(&mv)))
            {
                memset(vmove, 0, sizeof (*vmove));
    ...
    if (fp->base->hc)
    {
        fp->hv = (v_item *)calloc(fp->base->hc, sizeof (*fp->hv));
        fp->hc = fp->base->hc;
    ...
    if (fp->base->xc)
    {
        fp->xv = (v_swch *)calloc(fp->base->xc, sizeof (*fp->xv));
        fp->xc = fp->base->xc;
    ...
    if (fp->base->uc)
    {
        fp->uv = (v_ball *)calloc(fp->base->uc, sizeof (*fp->uv));
        fp->uc = fp->base->uc;
...
int sol_vary_cmd(struct s_vary *fp, struct cmd_state *cs, const union cmd *cmd)
{
    ...
    {
    case CMD_MAKE_BALL:
        if ((up = (v_ball *)realloc(fp->uv, sizeof (*up) * (fp->uc + 1))))
        {
            fp->uv = up;
            cs->curr_ball = fp->uc;
            fp->uc++;
...
int sol_lerp_cmd(struct s_lerp *fp, struct cmd_state *cs, const union cmd *cmd)
{
    ...
    {
    case CMD_MAKE_BALL:
        if ((uv = (l_ball(*)[2])realloc(fp->uv, sizeof (*uv) * (fp->uc + 1))))
        {
            fp->uv = uv;
...
int sol_load_lerp(struct s_lerp *fp, struct s_vary *vary)
{
    ...
    if (fp->vary->mc)
    {
        fp->mv = (l_move(*)[2])calloc(fp->vary->mc, sizeof (*fp->mv));
        fp->mc = fp->vary->mc;
    ...
    if (fp->vary->uc)
    {
        fp->uv = (l_ball(*)[2])calloc(fp->vary->uc, sizeof (*fp->uv));
        fp->uc = fp->vary->uc;
