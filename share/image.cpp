...
* Create an OpenGL texture object using the given image buffer.
*/
pgl::Texture2D make_texture_pgl(const void *p, int w, int h, int b, int fl)
{
    static const GLenum format[] =
    { 0, GL_LUMINANCE, GL_LUMINANCE_ALPHA, GL_RGB, GL_RGBA };
    ...
    o = glcontext->createTexture2D();
    o->wrap<0>(pgl::TextureWrap::ClampToEdge);
    o->wrap<1>(pgl::TextureWrap::ClampToEdge);
    o->magFilter(pgl::TextureFilter::Linear);
    o->minFilter(pgl::TextureFilter::Linear);

    /*
    pgl::TextureStorageDescriptor tsd;
    tsd.internalFormat = (pgl::InternalFormat)format[b];
    tsd.size = {W, H, 1};

    o->storage(tsd);

    pgl::TextureSubImageDescriptor tsdd;
    tsdd.textureFormat = (pgl::TextureFormat)format[b];
    tsdd.textureType = pgl::Type::UnsignedByte;
    tsdd.size = { W, H, 1 };
    tsdd.data = (void *)(q ? q : p);

    o->subImage(tsdd);
    */

    glBindTexture(GL_TEXTURE_2D, o->glID());
...
pgl::Texture2D make_image_from_file_pgl(const char *filename, int fl)
{
    void  *p;
    int    w;
    int    h;
