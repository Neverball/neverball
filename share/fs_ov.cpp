...
{
    return fs_read(ptr, size, nmemb, (fs_file)datasource);
}

int fs_ov_seek(void *datasource, ogg_int64_t offset, int whence)
{
    return fs_seek((fs_file)datasource, offset, whence);
}

int fs_ov_close(void *datasource)
{
    return fs_close((fs_file)datasource);
}

long fs_ov_tell(void *datasource)
{
    return fs_tell((fs_file)datasource);
}
