...
static void sol_draw_bill(const s_draw *draw, const int mi, GLboolean edge)
{
    ...
    //intersector->loadMesh(meshloader);
    //intersectorBillboard->loadMesh(meshloader);
    ...
    const struct s_draw *draw, int mi)
{
    ...
    pgl::floatv * vv = vert.data();//&vert[0];
    pgl::floatv * nv = norm.data();//&norm[0];
    pgl::floatv * tv = tex.data();//&tex[0];
    d_geom * gv = (d_geom *)idc.data();//&idc[0];
    pgl::intv * iv = (pgl::intv *)remidc.data();//&remidc[0];
    ...
    //meshloader->setIndices(idcBuf);
    //meshloader->setTexcoords(texbuf);
    //meshloader->setNormals(normbuf);
    //meshloader->setVertices(vertbuf);
...
void sol_draw_mesh(const struct d_mesh *mp, struct s_rend *rend, int p)
{
    ...
    if (sol_test_mtrl(mp->mtrl, p))
    {
        ...
        glBindBuffer_(GL_ELEMENT_ARRAY_BUFFER, mp->idcBuf->glID());

        glBindBuffer_(GL_ARRAY_BUFFER, mp->vertBuf->glID());
        glVertexPointer(3, T, sizeof(float) * 3, 0);

        glBindBuffer_(GL_ARRAY_BUFFER, mp->normBuf->glID());
        glNormalPointer(T, sizeof(float) * 3, 0);
        ...
        if (tex_env_stage(TEX_STAGE_SHADOW))
        {
            glTexCoordPointer(3, T, sizeof(float) * 3, 0);
            if (tex_env_stage(TEX_STAGE_CLIP)) {
                glTexCoordPointer(3, T, sizeof(float) * 3, 0);
            }
            tex_env_stage(TEX_STAGE_TEXTURE);
        ...
        */
        ...
        glBindBuffer_(GL_ARRAY_BUFFER, mp->texBuf->glID());
        glTexCoordPointer(2, T, sizeof(float) * 2, 0);
