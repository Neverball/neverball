...
static int set_hw_params(snd_pcm_t *pcm, snd_pcm_hw_params_t *p)
{
	...
	CK(snd_pcm_hw_params_set_buffer_time_near(pcm,
	                                     p, &buffer_time, &dir));
	CK(snd_pcm_hw_params_get_buffer_size(p, &buffer_size));

	/* Set the period time. */

	CK(snd_pcm_hw_params_set_period_time_near(pcm,
	                                     p, &period_time, &dir));
...
/* The audio system is ready for another block. Mix one and queue it. */
/* This function is invoked asynchonously by libasound in response to */
/* SIGIO.                                                             */
...
/* Initialize the audio system and start the asynchronous update.     */
/* Returns < 0 on failure.  snd_errstring describes the error.        */
...
/* Add the given buffer to the voice list for playback.  Len is the   */
/* buffer length in frames (NOT bytes).  Vol is an amplitude scaler.  */
/* The application must handle buffer management.                     */
...
/* Load the named file into a newly allocated buffer.  Len gets the   */
/* length of the buffer in frames.  Returns NULL on failure.          */
